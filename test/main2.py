...
out = cv2.VideoWriter(f'result/swing golf {n}.mp4', fourcc, fps, (W, H))
...
while cap.isOpened():
    ...
    if results[0].boxes.shape[0] > 0:  # Si hay detecciones
        ...
        confidences = results[0].boxes.conf.cpu().numpy()  # Confianzas

        # Filtrar solo las personas (clase 0 en COCO)
        person_indices = np.where(class_ids == 0)[0]
        if len(person_indices) > 0:
            # Calcular el área de cada cuadro delimitador y multiplicar por la confianza
            areas = [(boxes[i][2] - boxes[i][0]) * (boxes[i][3] - boxes[i][1]) for i in person_indices]
            scores = [areas[i] * confidences[i] for i in range(len(person_indices))]

            # Seleccionar la persona con la puntuación más alta
            closest_person_idx = person_indices[np.argmax(scores)]
            keypoints = results[0].keypoints.xy.cpu().numpy()[closest_person_idx]
            box = boxes[closest_person_idx]

            # Crear el marco anotado manualmente
            annotated_frame = frame.copy()
            cv2.rectangle(annotated_frame, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), COLORS['impact'], 2)
...
print("\n--- REPORTE FINAL ---")
print(f"Velocidad de Impacto Máxima: {safe_value(metrics['max_impact_speed']) * (1/100):.1f} m/s")
print(f"Velocidad de la Pelota: {safe_value(metrics['ball_speed']) * (1/100):.1f} m/s")
print(f"Velocidad Media Global: {safe_value(np.mean(list(metrics['average_velocities'].values()))) * (1/100):.1f} m/s")
print("\nRango de Movimiento (ROM):")
for joint in ['shoulder', 'elbow', 'wrist', 'hip', 'knee', 'ankle']:
    min_ang, max_ang = metrics['rom'][joint]
    rom_range = safe_value(max_ang - min_ang)
    ...
    print(f"{joint.capitalize()}: {dist:.1f} m")
print("\nVelocidades por Articulación:")
for joint in ['right_shoulder', 'right_elbow', 'right_wrist', 'right_hip', 'right_knee', 'right_ankle']:
    print(f"{joint.capitalize()}: Max={safe_value(metrics['max_velocities'][joint])* (1/100):.1f} m/s, Avg={safe_value(metrics['average_velocities'][joint])* (1/100):.1f} m/s")