...
class DeadliftExercise(Exercise):
    ...
    def __init__(self, side: str, user_height: float):
        ...
        # Nuevo: Almacenar alturas del bounding box para calcular el MÁXIMO
        self.bbox_heights = []  
        self.calibration_frames_needed = 20  # Número de frames para calibración
    ...
    def update(self, keypoints, confs: float, current_time):
        ...
        if self.disp_threshold is None:
            if valid_keypoints(confs, [wrist_idx, ankle_idx]):
                ...
                if len(self.calib_disp) >= 5:
                    ...
                    self.disp_threshold = (max_disp_px * self.pixel_scale) - 0.14
        ...
        if valid_keypoints(confs, [wrist_idx, ankle_idx]):
            wrist = keypoints[wrist_idx]
            ankle = keypoints[ankle_idx]
            space = (((self.user_height * 0.06))/40) *(180/ 3.14159)  # Ajuste de espacio para el Deadlift
            displacement_px = abs(ankle[1] - wrist[1] - space*2 )
